...
const TOKEN = process.env.AFFORDMED_LOG_TOKEN ?? process.env.AFFORDMED_API_TOKEN ?? '';
...
const server = http.createServer(async (request, response) => {
  ...
  await safeLog('backend', 'info', 'route', `${method} ${pathname} request received`);

  try {
    if (method === 'GET' && pathname === '/api/health') {
      sendJson(response, 200, { ok: true, message: 'Local API is running' }, startedAt);
      return;
    }

    if (method === 'POST' && pathname === '/api/logs') {
      const body = await readRequestBody(request);
      const result = await Log(body.stack, body.level, body.package, body.message);
...
server.listen(PORT, async () => {
  await safeLog('backend', 'info', 'service', `local API started on port ${PORT}`);
